<div class="flex-1 flex flex-col gap-6 sm:gap-8 items-center justify-center px-4 sm:px-6">
  <form method="POST" action="{{ route('intention.create', ['user_id' => auth()->user()->id, 'church_id' => $churchId]) }}" class="grid gap-4 md:grid-cols-2 w-full max-w-4xl">
    @csrf
    <div class="space-y-2">
    ...
    <div class="space-y-2 col-span-2">
      ...
      <div class="flex flex-column base:flex-row flex-wrap space-y-4 base:space-y-0 items-center justify-between">
        <button id="dropdownDefaultButton" data-dropdown-toggle="dropdown" class="text-zinc-500 bg-zinc-900 justify-between w-full focus:ring-2 focus:outline-none font-medium rounded-md text-base px-5 py-4 text-center inline-flex items-center" type="button">
          {{ $churchName ? $churchName : 'Selecionar comunidade' }}
        ...
        <div id="dropdown" class="z-10 hidden divide-y divide-zinc-100 rounded-lg border border-zinc-600 shadow w-[70%] bg-zinc-950">
